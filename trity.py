...
if '3.0.1' not in r.text:
    print (''+R+'\nYou need to update! The newest version is: ' + color.BOLD + color.UNDERLINE + r.text + '\n')
    update = raw_input(''+T+'' + color.UNDERLINE + 'Would you like to download the newest version now>' + color.END)
    if update == "yes":
        url = 'http://www.toxic-ig.weebly.com/uploads/8/4/0/3/84036514/trity.zip'
        print ""+G+"[*] Downloading..."
        urllib.urlretrieve(url, "trity.zip")
        print ""+G+"[*] Saved to the working directory!"
    if update == "y":
        url = 'http://www.toxic-ig.weebly.com/uploads/8/4/0/3/84036514/trity.zip'
        print ""+G+"[*] Downloading..."
        urllib.urlretrieve(url, "trity.zip")
